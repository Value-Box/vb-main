...
const filteredProducts=activeCategory==='All'?categoryProducts:categoryProducts.filter(item=>item.category===activeCategory)
 ...
 const [isExiting, setIsExiting] = useState(false);
    ...
    const [cartCount, setCartCount] = useState(3); // Start from min 3
    const minCart = 3;
  
    // Progress calculation (3 products par full ho jaye)
    const progress = Math.min((cartCount / minCart) * 100, 100);

  return (
    <>
    <div className='flex bg-[#FCFCFC] mx-auto max-w-[1920px] w-full'>
      ...
      <div className="md:flex items-center space-x-2">
        ...
        <button className="hidden md:block bg-[#EEA500] text-black text-base 2xl:text-md border-[2px] 2xl:border-[3px] border-[#F8DB99] px-3 py-0.5 rounded-md font-semibold">
          Pick & Save
        </button>
        <span className="block md:hidden text-sm">Free Shipping - 9 Days Delivery</span>
      </div>
      <button className="block md:hidden bg-[#EEA500] text-black text-base 2xl:text-md border-[2px] 2xl:border-[3px] border-[#F8DB99] px-3 py-0.5 rounded-md font-semibold">
          Pick & Save
        </button>
      {/* Right Side Content */}
      <span className="hidden md:block text-xl 2xl:text-2xl">Free Shipping - 9 Days Delivery</span>
    </div>
    <div className='mt-3'>
    <h3 className='flex items-center gap-1 text-[#FB6514] font-semibold text-xl 2xl:text-3xl px-4 2xl:px-6 '><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
  <path fillRule="evenodd" clipRule="evenodd" d="M12.1111 1.41768C11.1479 2.11276 10.3636 3.02665 9.82277 4.08413C9.28191 5.1416 8.99994 6.31241 9.00007 7.50018V7.54818C9.00607 8.44818 9.17107 9.31218 9.46657 10.1117C9.70657 10.7537 9.16057 11.4767 8.55157 11.1617C7.41885 10.5751 6.45446 9.70932 5.74957 8.64618C5.44657 8.19018 4.77757 8.10168 4.47757 8.55918C3.48831 10.0653 2.97393 11.8337 3.00102 13.6354C3.02811 15.4372 3.59542 17.1893 4.62951 18.665C5.6636 20.1407 7.11685 21.272 8.80114 21.9125C10.4854 22.5529 12.3232 22.6731 14.0765 22.2573C15.8298 21.8416 17.418 20.9091 18.6354 19.5805C19.8528 18.252 20.6434 16.5887 20.9048 14.8058C21.1662 13.0229 20.8864 11.2026 20.1016 9.58051C19.3169 7.95843 18.0632 6.60927 16.5031 5.70768L16.4971 5.69718C14.9911 4.82975 13.8348 3.46438 13.2271 1.83618C13.0531 1.37118 12.5116 1.12968 12.1111 1.41768ZM13.2811 10.6832C13.9885 10.893 14.6336 11.2735 15.1594 11.7912C15.6852 12.3089 16.0757 12.9479 16.2965 13.652C16.5173 14.3561 16.5616 15.1037 16.4255 15.8289C16.2895 16.5542 15.9772 17.2349 15.5162 17.8111C15.0552 18.3873 14.4597 18.8413 13.782 19.1332C13.1042 19.4251 12.3652 19.5459 11.6298 19.4851C10.8944 19.4242 10.1852 19.1834 9.56477 18.784C8.94429 18.3846 8.43151 17.8388 8.07157 17.1947C7.76407 16.6472 8.39407 16.1537 9.00157 16.3097C9.92131 16.5468 10.8842 16.5623 11.8111 16.3547C12.2431 16.2572 12.4306 15.7757 12.2926 15.3557C12.0981 14.7559 11.9994 14.1292 12.0001 13.4987C12.0001 12.6212 12.1891 11.7887 12.5266 11.0372C12.5879 10.8967 12.6984 10.7834 12.8373 10.7185C12.9762 10.6536 13.134 10.6401 13.2811 10.6832Z" fill="#FB6514"/>
</svg> Hot Picks For You</h3>

<div className="relative w-full mx-auto mt-6 px-4">
    {/* Left Scroll Button */}
    <button
      onClick={scrollLeft}
      className="absolute left-2 z-50 top-1/2 -translate-y-1/2 h-20 hidden md:block bg-[rgba(26,26,26,0.80)] text-white rounded-full shadow-md"
    >
  ...
  <path d="M14.2893 18.2929C13.8988 18.6834 13.2656 18.6834 12.8751 18.2929L7.9877 13.4006C7.2073 12.6195 7.2076 11.3537 7.9883 10.5729L12.8787 5.68254C13.2692 5.29202 13.9024 5.29202 14.2929 5.68254C14.6834 6.07307 14.6834 6.70623 14.2929 7.09676L10.1073 11.2824C9.7167 11.6729 9.7167 12.3061 10.1073 12.6966L14.2893 16.8787C14.6798 17.2692 14.6798 17.9023 14.2893 18.2929Z" fill="#FCFCFC"/>
    ...
    <div
      ...
      className="flex overflow-x-auto space-x-4 md:px-4 py-2 active:cursor-grabbing scrollbar-hidden"
      onMouseMove={onDragProdImgScroll}
    >
      {products.map((product) => (
        <div
          key={product.id}
          className="min-w-[150px] md:min-w-[220px] bg-white rounded-lg hover:shadow-lg md:p-3"
        >
          ...
          />
          <button className='absolute bottom-2 right-2 bg-white border border-[#CCD1D2] rounded-md md:rounded-[10px] md:p-2 '><svg xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 25 24" fill="none">
  <path d="M18.4004 12.998H13.4004V17.998C13.4004 18.2633 13.295 18.5176 13.1075 18.7052C12.92 18.8927 12.6656 18.998 12.4004 18.998C12.1352 18.998 11.8808 18.8927 11.6933 18.7052C11.5057 18.5176 11.4004 18.2633 11.4004 17.998V12.998H6.40039C6.13517 12.998 5.88082 12.8927 5.69328 12.7052C5.50575 12.5176 5.40039 12.2633 5.40039 11.998C5.40039 11.7328 5.50575 11.4785 5.69328 11.2909C5.88082 11.1034 6.13517 10.998 6.40039 10.998H11.4004V5.99805C11.4004 5.73283 11.5057 5.47848 11.6933 5.29094C11.8808 5.1034 12.1352 4.99805 12.4004 4.99805C12.6656 4.99805 12.92 5.1034 13.1075 5.29094C13.295 5.47848 13.4004 5.73283 13.4004 5.99805V10.998H18.4004C18.6656 10.998 18.92 11.1034 19.1075 11.2909C19.295 11.4785 19.4004 11.7328 19.4004 11.998C19.4004 12.2633 19.295 12.5176 19.1075 12.7052C18.92 12.8927 18.6656 12.998 18.4004 12.998Z" fill="black"/>
          ...
          <p className='hidden md:inline-flex items-center select-none rounded-md bg-gradient-to-l from-white to-[#FFD2AE] mt-1 ml-2'>
               ...
               <span className="flex md:hidden items-baseline font-semibold text-[#F04438] select-none">
                   <span className="text-xs">PKR</span>
                   <span className="text-lg ml-[2px]">{product.price}</span>
                   <del className='text-[#999] ml-1'>7,699</del>
                 </span>
                 
          <div className="flex items-center space-x-1 text-yellow-500 text-sm select-none">
            ⭐ {product.rating}
            <span className="text-gray-500 text-xs">{product.sold}</span>
          </div>
          </div>
        </div>
      ))}
    ...
    <button
      onClick={scrollRight}
      className="absolute right-2 top-1/2 -translate-y-1/2 h-20 hidden md:block bg-[rgba(26,26,26,0.80)] text-white rounded-full shadow-md"
    >
     <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
...
<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-3 mt-6 mb-6 px-2 md:px-4">
        {filteredProducts.map((product,index) => (
          <div className=" rounded-lg hover:shadow-md " key={index}>
...
<stop offset={`${(product.rating % 1) * 100}%`} stopColor="#ccc" />
      ...
      <div className='px-2 xl:px-4 py-3 bg-[#FCFCFC] sticky top-[6%] shadow-md h-[94vh]'>
        
        <p className='text-base lg:text-lg 2xl:text-xl'>Pick 3 Items for free Shipping</p>
        <div className='flex gap-2 mt-3'>
          <p className='bg-[#E6EAF3] w-full h-2 rounded-full '></p>
          <p className='bg-[#E6EAF3] w-full h-2 rounded-full '></p>
          <p className='bg-[#E6EAF3] w-full h-2 rounded-full '></p>
        </div>

        <div className='bg-[#F2F2F2] p-2 rounded-lg flex gap-2 mt-3 text-[#999] text-base lg:text-lg 2xl:text-xl '>
          <p className='bg-[#FFF] text-black rounded-full px-3 py-1 '>0</p>
          <p>Checkout</p>
          <p>PKR</p>
        </div>
        ...
        <div className='flex flex-col justify-center h-full'>
          ...
          <p className='text-base lg:text-lg 2xl:text-xl text-center'>No items yet? Continue shopping to explore more!</p>
        </div>
      </div>
      </div>
      
    </div>
    <div className='fixed bottom-0 block md:hidden w-full px-4'>
        <div className='bg-[#FCEDCD] p-4 rounded-t-[10px] '>
          <h3 className='font-semibold'>Pick 3 Items for free Shipping</h3>
          <div className="flex items-center bg-[#002882] rounded-[10px] px-4 py-2 w-full ">
      {/* Currency Label */}
      <span className="text-white font-medium mr-3">PKR</span>
      ...
      <div className="relative flex-1 h-2 bg-gray-200 rounded-full overflow-hidden">
        ...
        <div
          ...
          style={{
            width: `${progress}%`,
            background:
              "linear-gradient(90deg, #FFC136 0%, #FAC142 19.76%, #FFD168 52.5%, #F5BC3A 77.13%, #E09B00 100%)",
          }}
        ></div>
      </div>
      ...
      {/* Checkout Button */}
      <button
        className="ml-3 px-4 py-1 rounded-full text-black font-medium shadow-md"
        style={{
          background:
            "linear-gradient(90deg, #FFC136 0%, #FAC142 19.76%, #FFD168 52.5%, #F5BC3A 77.13%, #E09B00 100%)",
        }}
